...
class Timer {
   public:
    ...
    template <class F, class... Args>
    std::optional<std::size_t> SetTimeout(int milliseconds, F&& function,
                                          Args&&... arguments) {
                           ...
                           std::chrono::high_resolution_clock::now();
        {
            ...
            auto schedule_iter =
                schedule_.insert(std::make_pair(expire_time, new_handler));
        ...
        timer_worker_ = std::thread([this]() {
            while (true) {
                std::shared_ptr<class TimerHandler> handler;
                {
                    ...
                    // May be cancelled when sleeping, see if it is still in the
                    // event_queue.
