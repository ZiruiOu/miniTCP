...
EthernetDevice::EthernetDevice(const std::string& device_name)
    ...
    pcap_handler_ = pcap_create(device_name.c_str(), err_buffer);
    MINITCP_ASSERT(pcap_handler_) << "pcap_create error: " << device_name_
                                  << " " << err_buffer << std::endl;

    // timeout
    status = pcap_set_timeout(pcap_handler_, -1);
    MINITCP_ASSERT(status == 0)
        << "pcap_set_timeout error: " << device_name_ << " " << std::endl;

    // pcap buffer length
    status = pcap_set_snaplen(pcap_handler_, 3000);
    MINITCP_ASSERT(status == 0)
        << "pcap_set_timeout: " << device_name_ << " " << std::endl;
    ...
    MINITCP_ASSERT(status == 0)
        << "pcap_set_promisc error: " << device_name_ << " " << std::endl;
    ...
    MINITCP_LOG(DEBUG) << "EthernetDevice " << device_name_
...
// TODO : add definition of ip_addr
EthernetDevice::EthernetDevice(const std::string& device_name,
                               const mac_t& mac_addr)
    : device_name_(device_name), mac_addr_(mac_addr) {
    char err_buffer[PCAP_ERRBUF_SIZE];
    pcap_handler_ =
        pcap_open_live(device_name_.c_str(), 3000, 1, 5000, err_buffer);
