...
int EthernetDevice::SendFrame(const std::uint8_t* buffer, std::size_t length,
  ...
  {
    auto frame =
        std::shared_ptr<std::uint8_t>(new std::uint8_t[18 + length](),
  ...
  if (status == -1) {
    ...
    MINITCP_LOG(ERROR) << "sendframe: " << device_name_ << " send frame fail "
                       << err_buffer << std::endl;
  }
  return status;
}
...
int EthernetDevice::InitMacAddress() {
  ...
  if (!found) {
    MINITCP_LOG(ERROR) << "EthernetDevice: cannot initialize mac address for "
...
int EthernetDevice::InitIpAddress() {
  ...
  freeifaddrs(ifa_begin);
  if (!found) {
    MINITCP_LOG(ERROR) << "EthernetDevice : cannot initialize ip address for "
                       << device_name_ << std::endl;
  }
  return -(found == 0);
}
...
int EthernetDevice::InitPcapHandler() {
  ...
  MINITCP_ASSERT(status == 0) << "pcap_set_timeout error: " << device_name_
                              << " " << pcap_geterr(pcap_handler_) << std::endl;

  // pcap buffer length
  status = pcap_set_snaplen(pcap_handler_, kPcapSnapLen);
  MINITCP_ASSERT(status == 0) << "pcap_set_spanlen error: " << device_name_
                              << " " << pcap_geterr(pcap_handler_) << std::endl;
  ...
  MINITCP_ASSERT(status == 0) << "pcap_set_promisc error: " << device_name_
                              << " " << pcap_geterr(pcap_handler_) << std::endl;
  ...
  // set direction : only received
  status = pcap_setdirection(pcap_handler_, PCAP_D_IN);
  MINITCP_ASSERT(status == 0) << "pcap_setdirection error: " << device_name_
                              << " " << pcap_geterr(pcap_handler_) << std::endl;

  // set non blocking
  status = pcap_setnonblock(pcap_handler_, 1, err_buffer);
  MINITCP_ASSERT(status == 0)
      << "pcap_setnonblock error: " << device_name_ << err_buffer << std::endl;
