...
void RoutingTable::InitRoutingTable() {
  ...
  for (netif = netif_all; netif != NULL; netif = netif->next) {
    ...
    if (device_id != -1) {
      ...
      MINITCP_LOG(INFO) << "RoutingTable initRoutingTable : add device "
                        << netif->name << "into the routing table" << std::endl;
  ...
  if (distance < kRipPoisonThresh) {
    switch (status) {
      case kStable:
        ...
        break;
      case kPersist:
        routing_table_.push_back(
            RoutingEntry{kPersist, 0, 0, dest, netmask, nexthop_ip, distance});
        MINITCP_LOG(INFO) << "unsafeInsert: successfully add an "
                             "permanent item into the routing table."
                          << std::endl;
        break;
      default:
        MINITCP_LOG(ERROR) << "unsafeInsert: no such routing entry status. "
                           << std::endl;
        break;
    }
  }
